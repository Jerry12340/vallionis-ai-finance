...
if not all([stripe.api_key, STRIPE_PRICE_ID, STRIPE_LIFETIME_PRICE_ID]):
    logger.warning("Missing Stripe configuration in environment variables")
...
class User(db.Model, UserMixin):
    id = db.Column(db.Integer, primary_key=True)
    email = db.Column(db.String(120), unique=True, nullable=False)
    password = db.Column(db.String(255), nullable=False)  # Changed from 60 to 255
    premium = db.Column(db.Boolean, default=False)
    subscription_expires = db.Column(db.DateTime)
    stripe_customer_id = db.Column(db.String(50))
    subscription_type = db.Column(db.String(20), default=None)
    ...
    def get_subscription_status(self):
        if self.premium:
            # Grant access for lifetime subscribers
            if self.subscription_type == 'lifetime':
                return True
            # Grant access for trialing or active subscriptions
            elif self.subscription_status in ['trialing', 'active']:
                # Check if subscription_expires is set and valid
                if self.subscription_expires and self.subscription_expires > datetime.utcnow():
                    return True
                else:
        ...
        if form.validate_on_submit():
            try:
                results = process_request(
                    ...
                    form.premium.data if current_user.get_subscription_status() else False,  # Fixed here
                    form.sector_focus.data if current_user.get_subscription_status() else 'all',  # Fixed here
                    form.risk_tolerance.data if current_user.get_subscription_status() else 'medium',  # Fixed here
                    form.dividend_preference.data if current_user.get_subscription_status() else False  # Fixed here
                )
                # Ensure results contains all required keys
...
@app.route('/purchase-lifetime', methods=['GET', 'POST'])  # Add POST method
@login_required
def purchase_lifetime():
    ...
    if request.method == 'POST':  # Only process Stripe checkout on POST
        try:
            checkout_session = stripe.checkout.Session.create(
                client_reference_id=current_user.id,
                ...
                payment_method_types=['card'],
                line_items=[{
                    'price': os.getenv('STRIPE_TEST_LIFETIME_PRICE_ID'),
                    'quantity': 1,
                }],
                mode='payment',
                success_url=url_for('success', _external=True) + '?session_id={CHECKOUT_SESSION_ID}',
            ...
            print(f"Lifetime purchase error: {str(e)}")
            flash('Error processing payment', 'danger')
            return redirect(url_for('subscription'))

    # For GET requests, just render the subscription page
    return redirect(url_for('subscription'))
